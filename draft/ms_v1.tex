...
\begin{abstract}
    ...
    We use the coadd images in two \tracts{} from the Wide layer with representative
    seeing conditions to demonstrate the usage of \synpipe{} and examine the general
    photometric performance of \texttt{hscpipe}.
    ...
    mag (1\%-5\% accuracy in the $i$-band).
    For synthetic single-{S\'{e}rsic\ } model galaxies, the typical uncertainties of
    HSC forced \cmodel{} photometry range from 0.14 mag at $i{\sim}20.0$ mag to
    0.20 mag at $i{\sim} 25.2$ mag.
    Over the range of colors and magnitudes in these tests, we find that both \forced{}
    PSF and \cmodel{} photometry provide unbiased estimates of colors that are robust
    against different seeing conditions.
    Finally, we also noticed that 1) the degree that a object is blended with others
    impact the photometric accuracy, especially for stars.
    ...
    2) a relative high fraction of faint stars (${\sim}20$\% at $i>22.5$ mag) could be
...
\subsubsection{Input magnitude and the $\mathrm{S}/\mathrm{N}$ of PSF photometry}
    ...
    In $i$-band, the highly blendend synthetic stars scatter toward lower \s2n{} at
    fixed magnitude.
    Interestingly, the effect is not obvious in $g$-band, suggeting the deblending
...
\subsubsection{Accuracy of PSF magnitude}
    ...
    magnitud keeps at ${\sim}3$\% level (${\sim}0.036$ mag average difference).
    Down to $i_{\mathrm{Input}}{\sim}25.2$ mag, the average accuracy descreases to
    ...
    differenecs in seeing), it will not affect the accuracies of magnitudes of colors.
    ...
    increasingly significant at fainter magnitde.
