...
\begin{abstract}
   ...
   The Subaru Strategic Program (SSP) is an ambitious wide-area, multi-band
    survey using the Hyper Suprime-Cam (HSC) on the Subaru telescope.
      At SSP's imaging depth \susan{quantify?}, it is challenging to achieve accurate,
    unbiased, and consistent photometry across all five bands. To improve data processing and achieve accurate photometric measurements, we have developed
   \synpipe{}, a \texttt{Python}-based, flexible framework
    that interfaces with the HSC data reduction pipeline (\hscpipe{}) to process data from the first public HSC--SSP data release. \susan{I expected to see something in the abstract that clarifies why this is Pipeline I; does this suggestion accomplish that?} We test  \synpipe{}
    performance by injecting synthetic objects into single-\texttt{visit} HSC images.
    We use coadded images in two Wide layer \tracts{} with representative
    seeing conditions to demonstrate  \synpipe{} implementation and to examine  \texttt{hscpipe}'s 
    ...
    forced \cmodel{} photometry typically range from 0.14 mag at $i{\sim}20.0$ mag to
    0.20 mag at $i{\sim} 25.2$ mag.
    Over the color and magnitude ranges used in the  \synpipe{} tests, we find that both 
    \forced{} PSF and \cmodel{} photometry provide unbiased color estimates that 
