...
\subsection{Running the \synpipe{} Tests}
    ...
    the Universe (Kavli IPMU) to conduct the \synpipe{} tests on the cluster 
    (\texttt{Master}).
    ...
    ``ambiguously blended'' objects, which are defined as so extreme blends that 
    some objects are detected as a single object.
    The ambiguously blended objects are useful in the WL-related statistical tests 
    (see Murata et al. in prep. for more details); but since they will not help test 
    photometry, they will be excluded later.

    The detailed log of the entire tests can be found at \url{goo.gl/VINOVP}.
    The user should be able to reproduce the results presented in this work using 
    this information. It can also serve as a brief manual for running a \synpipe{} 
    test.
...
\section{Photometric Performance Results}
    ...
    and are consistent across all bands in terms of position and shape. T
    Therefore, they are the most appropriate and relevant methods for star and galaxy.
    \song{Edit stops here}
%% ------------------------------------------------------------------------------------ %%
...
\begin{figure*}
    ...
    \caption{
        ...
        The gray dashed line marks $S/N = 5$.
        }
    \label{fig:star_sn}
\end{figure*}
%% ------------------------------------------------------------------------------------ %%
